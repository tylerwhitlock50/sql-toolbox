SELECT
orders.product_code,
orders.product_code as Product_Code_Filter,
Orders.Customer_ID as Customer_Filter,
orders.Part_ID,
orders.Part_ID as Part_ID_Filter,
Orders.ID,
Orders.Identifier,
Orders.Customer_ID,
Orders.DESIRED_SHIP_DATE,
case 
when orders.customer_po_ref = 'VIP' then 'V'
when orders.customer_po_ref = 'POINT REDEMPTION' then 'PT'
when orders.customer_po_ref = 'PROSTAFF' then 'PRO'
when orders.customer_po_ref = 'EMPLOYEE' then 'EMP'
else '' end as PO_Type,

--Hidden from the Shipping Operator
--Orders.Early_Flag,
Orders.composite_score,
--Orders.Date_score,
--Orders.Multiplier,
--Orders.Customer_Score,
left(address.Address_Comp,40) as Address_Comp,
Orders.Qty,
isnull(QOH.QTY,0) as QOH
...
case 					
	when Orders.desired_ship_Date	>	Getdate() + 5	then	'Early Shipment'  end	 as Early_Flag,
...
when stats.customer_ID = 'Bass Pro' then 2.5
...
when stats.customer_ID = 'SCHEELS' then 2.4
when stats.customer_ID = 'SPOR WARE' then 2.5
...
SELECT 
CUSTOMER_ORDER.ID, 
isnull(CUST_ADDRESS.NAME, CUSTOMER.NAME)
 + '
 ' + isnull(CUST_ADDRESS.CITY, CUSTOMER.CITY)
+ ', ' + isnull(CUST_ADDRESS.STATE, CUSTOMER.STATE) + ' ' + isnull(CUST_ADDRESS.ZIPCODE, CUSTOMER.ZIPCODE) + ' 
'+ isnull(CUST_ADDRESS.COUNTRY, CUSTOMER.COUNTRY) as Address_comp

 

FROM VECA.dbo.CUST_ADDRESS CUST_ADDRESS right join VECA.dbo.CUSTOMER_ORDER CUSTOMER_ORDER
on CUSTOMER_ORDER.CUSTOMER_ID = CUST_ADDRESS.CUSTOMER_ID AND CUSTOMER_ORDER.SHIP_TO_ADDR_NO = CUST_ADDRESS.ADDR_NO
left join 
veca.dbo.customer on customer.ID = CUSTOMER_ORDER.CUSTOMER_ID
) as address on address.ID = orders.ID

left join veca.dbo.customer_entity Customer_entity on orders.customer_id = customer_entity.customer_ID
left join (
...
) as QOH on QOH.part_ID = orders.part_ID

where 
Orders.Early_Flag is  null
and customer_entity.credit_status in ('A','O')
