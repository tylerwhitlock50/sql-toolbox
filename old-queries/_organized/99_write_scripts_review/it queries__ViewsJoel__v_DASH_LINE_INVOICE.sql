...
/****** Object:  View [ESI_EDI].[v_DASH_LINE_INVOICE]    Script Date: 9/8/2021 1:22:48 PM ******/
SET ANSI_NULLS ON
GO
...
CREATE VIEW [ESI_EDI].[v_DASH_LINE_INVOICE] 
									AS
									...
									FROM [3CARMS\CAPROD].[VECA].[dbo].[RECEIVABLE_LINE] ERP_RCL
										INNER JOIN [3CARMS\CAPROD].[VECA].[dbo].[RECEIVABLE] ERP_REC 
											ON ERP_REC.[INVOICE_ID]	= ERP_RCL.[INVOICE_ID]	
										LEFT JOIN [3CARMS\CAPROD].[VECA].[dbo].[SHIPPER_LINE] ERP_SHL 
											ON ERP_SHL.[PACKLIST_ID] = ERP_RCL.[PACKLIST_ID]
											AND ERP_SHL.[LINE_NO] = ERP_RCL.[PACKLIST_LINE_NO]	
										LEFT JOIN [3CARMS\CAPROD].[VECA].[dbo].[SHIPPER] ERP_SHH	
											ON ERP_SHH.[PACKLIST_ID] = ERP_SHL.[PACKLIST_ID]
										INNER JOIN [3CARMS\CAPROD].[VECA].[dbo].[CUSTOMER_ORDER] ERP_ORH 
											ON ERP_ORH.[ID] = ERP_SHH.[CUST_ORDER_ID] 
										INNER JOIN [3CARMS\CAPROD].[VECA].[dbo].[CUST_ORDER_LINE] ERP_ORL	
											ON ERP_ORL.[CUST_ORDER_ID] = ERP_SHL.[CUST_ORDER_ID]
											AND ERP_ORL.[LINE_NO] = ERP_SHL.[CUST_ORDER_LINE_NO]
										INNER JOIN [3CARMS\CAPROD].[VECA].[dbo].[PART] ERP_PRT 
											ON ERP_PRT.[ID]	= ERP_ORL.[PART_ID]	
										INNER JOIN [3CARMS\CAPROD].[VECA].[dbo].[SITE] SIT     
											ON SIT.[ID] = ERP_ORH.[SITE_ID]
										LEFT JOIN [3CARMS\CAPROD].[VECA].[dbo].[SALES_TAX] TAX	
