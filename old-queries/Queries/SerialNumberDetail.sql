select
latest_trans.trace_id,
latest_trans.max_trans,
...
max([TRACE_INV_TRANS].[TRANSACTION_ID]) as max_trans
FROM [VECA].[dbo].[TRACE_INV_TRANS]
group by 
[TRACE_INV_TRANS].trace_id) as latest_trans

inner join

veca.dbo.inventory_trans on inventory_trans.transaction_id = latest_trans.max_trans
...
on stock_style.DOCUMENT_ID = part.id) as UDFs

on UDFs.ID = inventory_trans.part_id