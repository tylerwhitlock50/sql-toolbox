/*
===============================================================================
Query Name: open_wo_list.sql

Purpose:
    Snapshot of all open work orders (on-time AND past-due), aged by how far
    past or before their desired want date they are, with WIP value and
    linkage back to any open customer-order demand driving them.

    Same row-set as open_wo_aging_and_wip.sql (which already includes both
    on-time and late WOs). Provided under the broader name for symmetry with
    open_po_list.sql and the broader-scope SSRS report (open_wo_list.rdl).
    The original open_wo_aging_and_wip.sql is preserved as the canonical
    name historical references depend on.

    Intended as the daily / weekly "what's open on the floor" report
