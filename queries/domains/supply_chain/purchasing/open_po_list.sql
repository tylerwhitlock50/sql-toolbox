/*
===============================================================================
Query Name: open_po_list.sql

Purpose:
    Line-level list of all open purchase-order lines (past-due AND not-yet-due),
    bucketed by aging and tagged with priority (P1-P4 per past-due/SO-linkage
    cross). Includes service POs.

    Broader-scope counterpart to ../performance/past_due_po_aging.sql (which
    filters to past-due only). Use this for a complete buyer working list;
    use the past-due variant for focused expediting.

Grain:
    One row per open PO line (delivery schedule if present).
    Open = line/header status not ('X','C') AND ORDER_QTY > RECEIVED_QTY.
    Includes service-only lines (PART_ID NULL but SERVICE_ID populated).
...
Priority:
    P1 - past due, on open SO (expedite)
    P2 - past due, no SO linkage
    P3 - not due, on open SO
    P4 - normal

Business Use:
    - Buyer's working list (past-due + everything else they own)
    - "Value at risk" by aging bucket
    - Service / blanket PO visibility (rendered as "service_id | part_id"
      in the SSRS report when both populated)

Notes:
    - Uses PO unit_price for value (in purchase UOM). For stock-UOM-normalized
      pricing, use open_and_planned_supply_detail.sql.
    - SO linkage is direct PART_ID match only; service lines never link to an SO.
    - For past-due-only output, see ../performance/past_due_po_aging.sql.
===============================================================================
*/
...
;WITH open_po AS (
    ...
    WHERE ISNULL(p.STATUS, '')        NOT IN ('X','C')
      ...
      AND (pl.PART_ID IS NOT NULL OR pl.SERVICE_ID IS NOT NULL)
